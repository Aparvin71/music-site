...
async function initAlbumsPage() {
    ...
    (Array.isArray(tracks) ? tracks : []).forEach(track => {
      ...
      if (!byAlbum.has(name)) {
        byAlbum.set(name, {
          ...
          cover: window.AineoShared?.resolveCoverPath ? window.AineoShared.resolveCoverPath(track.cover || track.cover_file || "") : (track.cover || ""),
  ...
  const normalizedAlbums = (Array.isArray(albums) ? albums : [])
    .map(album => ({
      ...
      cover: window.AineoShared?.resolveCoverPath ? window.AineoShared.resolveCoverPath(album.cover || "") : String(album.cover || "").trim(),
      year: album.year || "",
