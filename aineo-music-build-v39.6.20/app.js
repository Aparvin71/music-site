...
function clearAllFilters() {
  activeCustomPlaylistFilterName = null;
  filters.selectedAlbum = null;
  filters.selectedPlaylist = null;
  filters.selectedTag = null;
  filters.searchTerm = "";
...
function getActiveQueueSourceTracks() {
  if (activeCustomPlaylistFilterName && customPlaylists[activeCustomPlaylistFilterName]) {
    return getCustomPlaylistTracks(activeCustomPlaylistFilterName);
  }
...
function syncQueueToCurrentSource() {
  const sourceTracks = getActiveQueueSourceTracks();
  const currentTrack = getCurrentTrack();

  currentQueue = [...sourceTracks];

  if (!currentQueue.length) {
    currentQueueIndex = -1;
  } else if (currentTrack) {
    const matchedIndex = currentQueue.findIndex(track => track.id === currentTrack.id);
    currentQueueIndex = matchedIndex >= 0 ? matchedIndex : 0;
  } else {
    currentQueueIndex = 0;
  }
...
function getQueueDisplayTracks() {
  ...
  if ((activeCustomPlaylistFilterName || hasActiveLibraryFilter()) && Array.isArray(sourceTracks) && sourceTracks.length) {
    return sourceTracks;
  }
...
let activeCustomPlaylistFilterName = null;
...
function applyCustomPlaylistFilter(name) {
  const ids = customPlaylists[name] || [];
  activeCustomPlaylistFilterName = name;
  filteredTracks = ids.map(id => tracks.find(track => track.id === id)).filter(Boolean);
  filters.selectedAlbum = null;
  filters.selectedPlaylist = null;
  filters.selectedTag = null;
