/* v42.3.92 beat-accented mirrored halo */
...
function drawVisualizerFrame() {
  ...
  const sampleWave = (progress) => {
    ...
    const current = els.audioPlayer?.currentTime || 0;
    const pulse = Math.max(0, bass - 0.16);
    const layerA = Math.abs(Math.sin((visualizerTick / 10.5) + progress * Math.PI * 4 + current * 0.9));
    const layerB = Math.abs(Math.sin((visualizerTick / 5.4) + progress * Math.PI * 10 + current * 1.6 + 0.6));
    const layerC = Math.max(0, Math.sin((visualizerTick / 16) + progress * Math.PI * 2 - current * 0.7));
    const transient = pulse * (0.72 + layerB * 0.42);
    return 0.06 + layerA * (0.08 + mids * 0.10) + layerB * (0.04 + treble * 0.08) + layerC * 0.06 + transient;
  };

  for (let i = 0; i <= haloSteps; i += 1) {
    const progress = i / haloSteps;
    mirroredSamples.push(sampleWave(progress));
  }
  ...
  const drawHalo = (radiusBase, lineWidth, strokeStyle, shadowBlur, shadowColor, sampleScale) => {
    ...
    for (let i = 0; i <= haloSteps; i += 1) {
      ...
      const sampleIndex = Math.min(mirroredSamples.length - 1, Math.round(normalized * haloSteps));
      const waveAmount = mirroredSamples[sampleIndex];
      const taper = 0.78 + (Math.sin(progress * Math.PI) ** 1.5) * 0.22;
      const radius = radiusBase + waveAmount * maxWavePush * sampleScale * taper;
  ...
  visualizerBars.forEach((bar) => {
    ...
    const freqIndex = visualizerFreqData && !visualizerUseFallback
      ? Math.min(visualizerFreqData.length - 1, Math.floor((0.08 + mirrorIndex * 0.045) * visualizerFreqData.length))
      : -1;
