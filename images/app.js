...
function renderScriptureReferenceLinks(scriptures, options = {}) {
  const refs = Array.isArray(scriptures)
    ? scriptures.map(ref => String(ref || "").trim()).filter(Boolean)
    : [];

  if (!refs.length) {
    return options.emptyHtml || `<p class="empty-message">No scripture references available.</p>`;
  }

  const className = options.className || "scripture-link-list";
  const linkClassName = options.linkClassName || "scripture-ref-link";
  ...
  const links = refs.map(ref => {
    ...
    const safeUrl = escapeHtmlAttr(getScriptureUrl(ref));
    const safeClassName = escapeHtmlAttr(linkClassName);
    return `<a class="${safeClassName}" href="${safeUrl}" target="${target}" rel="${rel}" aria-label="Read ${safeRef} on BibleGateway">${safeRef}</a>`;
  }).join(separator);

  return `<div class="${escapeHtmlAttr(className)}">${links}</div>`;
}
        ...
        <div class="featured-track-row ${isPlaying}" data-track-id="${escapeHtmlAttr(track.id)}">
          ...
          <div class="featured-track-main">
            ...
            <div class="featured-track-meta-line">
              ...
              ${track.scripture_references.length ? `<span class="featured-track-scripture-links">• ${renderScriptureReferenceLinks(track.scripture_references, { className: "scripture-inline-links", linkClassName: "scripture-inline-link" })}</span>` : ""}
            </div>
          </div>
...
function updateNowPlaying(track) {
  ...
  if (els.nowScripture) {
    els.nowScripture.innerHTML = track.scripture_references.length
      ? renderScriptureReferenceLinks(track.scripture_references, {
          className: "scripture-inline-links",
          linkClassName: "scripture-inline-link"
        })
      : "—";
  }
...
function updateScripturePanel(track) {
  ...
  els.scriptureContent.innerHTML = renderScriptureReferenceLinks(track.scripture_references, {
    className: "scripture-block scripture-link-list",
    linkClassName: "scripture-ref-link"
  });
}
            ...
            <button class="album-track-main" data-album-track-index="${index}" type="button">
              ...
              ${
                track.scripture_references.length
                  ? `<div class="album-track-scripture">${renderScriptureReferenceLinks(track.scripture_references, { className: "scripture-inline-links", linkClassName: "scripture-inline-link" })}</div>`
                  : ""
              }
            </button>
            <div class="featured-track-actions">
