...
const AINEO_APP_VERSION = "v43.1.91";
    ...
    <div class="app-feel-surface-panel" role="dialog" aria-modal="true" aria-labelledby="appFeelSurfaceTitle">
      ...
      <div class="app-feel-surface-body">
        ...
        <section class="app-feel-group">
          ...
          <p class="page-lead compact-lead">Version <span class="app-version">v43.1.91</span></p>
...
function ensureFloatingSettingsButton() {
  // v43.1.91: Top-right floating app-feel/settings ellipsis removed by request.
...
function initPageMotionHooks() {
  ...
  if (pageShell) {
    pageShell.classList.add("page-enter", "page-enter-active");
  }
}
...
async function registerStandaloneServiceWorker() {
  ...
  await forceRuntimeCacheRefreshForCurrentBuild();
...
const APP_RUNTIME_VERSION = "v43.1.91";


const APP_SHELL_RESET_VERSION_KEY = "aineo_app_shell_reset_version";
const APP_SHELL_RESET_TARGET = "v43.1.91";
...
const LYRICS_LIBRARY_VERSION = "43.1.91";
...
async function forceRuntimeCacheRefreshForCurrentBuild() {
  ...
  try {
    ...
    await Promise.all(keys.map((key) => {
      const shouldDelete = (key.startsWith("static-") || key.startsWith("dynamic-") || key.includes("aineo-")) && !key.includes(APP_RUNTIME_VERSION);
