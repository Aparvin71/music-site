...
function drawVisualizerFrame() {
  ...
  const sampleWave = (progress) => {
    ...
    const layerA = Math.abs(Math.sin((visualizerTick / 10.5) + progress * Math.PI * 4 + currentTime * 0.9));
    const layerB = Math.abs(Math.sin((visualizerTick / 5.4) + progress * Math.PI * 10 + currentTime * 1.6 + 0.6));
    const layerC = Math.max(0, Math.sin((visualizerTick / 16) + progress * Math.PI * 2 - currentTime * 0.7));
    const transientPulse = pulse * (0.72 + layerB * 0.42);
    return 0.06 + layerA * (0.08 + mids * 0.10) + layerB * (0.04 + treble * 0.08) + layerC * 0.06 + transientPulse;
  };
