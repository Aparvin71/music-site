(function () {
  ...
  function renderMiniCard({ track, index, escapeHtml, escapeAttr }) {
    return `
      <button class="mini-card" data-mini-index="${index}" data-track-id="${escapeAttr(track.id)}" type="button">
        ${track.cover ? `<img src="${escapeAttr(track.cover)}" alt="${escapeAttr(track.title)} cover" class="mini-card-cover" />` : `<div class="mini-card-cover mini-card-placeholder">No Cover</div>`}
    ...
    return `
      <button class="album-card ${isActive ? "active" : ""}" data-album="${escapeAttr(album.name)}" type="button">
        <div class="album-card-cover-wrap">
          ${album.cover ? `<img class="album-card-cover" src="${escapeAttr(album.cover)}" alt="${escapeAttr(album.name)} cover" />` : `<div class="album-card-cover album-card-placeholder">No Cover</div>`}
        </div>
  ...
  window.AineoUI = { renderEmptyMessage, renderMiniCard, renderAlbumCard, renderPlaylistChipRow };
})();
